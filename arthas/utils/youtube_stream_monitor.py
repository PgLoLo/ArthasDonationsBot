...
class YoutubeStreamerMonitor:
    def __init__(self, username: str, api: YoutubeAPI):
        ...
        self.streamer_state: Optional[StreamerState] = None
    ...
    def check_if_stream_continues(self) -> None:
        assert self.streamer_state is not None
        assert self.streamer_state.video_id is not None

        video_status = self.api.get_video_info(self.streamer_state.video_id)
        ...
        if video_status.status == VideoStatus.Ended:
            self.streamer_state.video_id = None
            self.notify_stream_stopped()
        elif video_status.title != self.streamer_state.title:
            self.streamer_state.title = video_status.title
            self.notify_title_changed(video_status.title)

    def check_if_stream_started(self) -> None:
        assert self.streamer_state is not None
        assert self.streamer_state.video_id is None

        video_ids = (
            self.api.get_video_ids(self.streamer_state.playlist_id) +
            [self.api.get_video_id_from_feed(self.streamer_state.user_id)]
        )
        ...
        for video_status in video_statuses:
            if video_status.status == VideoStatus.Started:
                self.streamer_state.video_id = video_status.id
                self.streamer_state.title = video_status.title
    ...
    def run_loop(self) -> None:
        ...
        while not self.stopped:
            try:
                if self.streamer_state.video_id is not None:
